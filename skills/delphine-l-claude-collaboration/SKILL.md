---
name: delphine-l-claude-collaboration
...
For this team pack, create under `<repo>/skills/<name>/` and refresh with `/refresh-team`. Never copy a skill into `~/.claude/skills`, `~/.grok/skills`, `~/.gemini/skills`, or `.agents/skills` as a real directory.
