---
name: delphine-l-token-efficiency
description: Token optimization for coding agents. Filter before reading, prefer grep/head/bash for inspection, Read+Edit for code. Load a harness reference only for host-specific model routing. Use whenever reading logs, navigating a repo, or choosing between shell and file tools.
version: 1.5.0
...
This skill provides token optimization strategies for cost-effective coding-agent usage across all projects. These guidelines help minimize token consumption while maintaining high-quality assistance.
...
Use the host's cheaper/faster default for implementation and the host's stronger model for first-pass architecture learning or adversarial audit. Concrete names live in `references/harness-claude.md`, `references/harness-grok.md`, `references/harness-gemini.md`, and `references/harness-antigravity.md` — load only the file for the current host.
...
### 12. Use the host's read-only explore/plan subagent (Claude Task/Explore, Grok `explore`/`plan`, Gemini codebase investigator) for broad searches. Do not spawn from a child session.
...
2. **Low-cost operation** (< 100 lines output)? -> Use the current session
...
2. Is this a LOW-COST operation? (< 100 lines) -> Use the current session
...
- **LOW-COST operations** (< 100 lines) -> the current session
