---
name: delphine-l-claude-skill-management
...
## Multi-CLI install (this team pack)

For the Xcode Project Team repo, do **not** require `$CLAUDE_METADATA`. Canonical root is this git repo. Install/refresh with:

```bash
./scripts/refresh-team.sh
```

or, after the first link, `/refresh-team`. That script detects `claude`, `grok`, `gemini`, and `agy` and symlinks `skills/` and `agents/` into each present CLI. `$CLAUDE_METADATA` remains valid for Delphine's own `claude_global` layout; it is not the contract for this repo.
